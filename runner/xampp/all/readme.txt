...
1. 如何启动控制面板：
  双击启动start.exe运行控制面板。在控制面板集成运行环境控制面板上点击启动按钮即可启动。

  如果无法通过控制面板启动控制面板，进入xampp\service目录，双击运行install.bat，来安装并启动控制面板的服务。
...
2. 注意事项：
  ...
  2.3 控制面板系统默认的管理员帐号是admin，密码是123456。
  2.4 数据库默认的管理员帐号是root，密码为123456。
  2.5 控制面板的访问路径：http://localhost/ 其他机器访问将localhost换成ip地址。如果端口号不是80，还需要加上端口号。
  2.6 数据库管理请访问：http://localhost/phpmyadmin/。phpmyadmin只能在控制面板机器上面访问。

详细的介绍，请访问：http://www.cnezsoft.com/

Please read the flowing notes before you run chanzhi:

1. How to start chanzhi:

   Double Click start.exe to run control application。Click the start button to start services.
...
For more infomation, please visit: http://www.cnezsoft.com/.
